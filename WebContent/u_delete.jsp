...
<body>
 <%
	...
	Connection conn=DriverManager.getConnection(URL,USER,PASSWORD);//创建数据库连接
	 ...
	 String sql="delete from users where u_id="+deleteid;//定义插入SQL语句
	 int result=stmt.executeUpdate(sql);//执行SQL语句
	 if(result!=0){ 
	 response.sendRedirect("manage_user.jsp");//执行插入成功跳转
	 }else{
	 response.sendRedirect("manage_user.jsp");//执行插入失败跳转
	 }
    %>
</body>
</html>